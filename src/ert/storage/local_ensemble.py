...
class LocalEnsemble(BaseMode):
    ...
    def save_parameters(
        ...
        if isinstance(dataset, pl.DataFrame):
            ...
            logger.info(f"DEBUG save_parameters DataFrame input:\n{dataset.with_columns(pl.all().cast(pl.Utf8))}")
            ...
            try:
                ...
                logger.info(f"DEBUG save_parameters DataFrame final (before write):\n{df_full.with_columns(pl.all().cast(pl.Utf8))}")
            except KeyError:
                df_full = dataset
        ...
        logger.info(f"DEBUG save_parameters xarray input for group={group}, realization={realization}:\n{dataset['values'].values}")
        ...
        logger.info(f"DEBUG save_parameters xarray final (before write):\n{data_to_save['values'].values}")
        ...
        for (
            ...
            for real in reals:
                ...
                if response_type == "rft":
                    logger.info("This is RFT thing with cat")
